...
function solve() {
    return function (animals) {
        function printSpecies(animals, species) {
            console.log(Array(species.length + 2).join('-'));
            console.log(species + ':');
            console.log(Array(species.length + 2).join('-'));
        ...
        _.each(groupedAnimals, function(animals, species) {
           animals = _.chain(animals)
               .sortBy('name')
               .sortBy('legsCount')
               .value();

            printSpecies(animals, species);
        });
    };
}
